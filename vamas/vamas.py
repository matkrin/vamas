...
def read_vamas(f: TextIO) -> tuple[VamasHeader, List[VamasBlock]]:
    ...
    for _ in range(h.num_blocks):
        ...
        if h.experiment_mode in [
            ...
            b.sputtering_z = int(next(f)) if include[12] else fb.sputtering_z
            b.sputtering_num_particles = (
                float(next(f)) if include[12] else fb.sputtering_num_particles
            )
            b.sputtering_charge = (
                float(next(f).strip()) if include[12] else fb.sputtering_charge
            )
        ...
        b.analysis_source_beam_width_x = (
            float(next(f)) if include[15] else fb.analysis_source_beam_width_x
        )
        b.analysis_source_beam_width_y = (
            float(next(f)) if include[15] else fb.analysis_source_beam_width_y
        )
        ...
        if h.experiment_mode in [
            ...
            b.field_view_x = float(next(f)) if include[16] else fb.field_view_x
            b.field_view_y = float(next(f)) if include[16] else fb.field_view_y
        ...
        b.analyzer_mode = next(f).strip() if include[20] else fb.analyzer_mode
        ...
        b.species_label = next(f).strip() if include[28] else fb.species_label
        ...
        for _ in range(int(b.num_y_values / len(b.corresponding_variables))):
            for corres_var in b.corresponding_variables:
